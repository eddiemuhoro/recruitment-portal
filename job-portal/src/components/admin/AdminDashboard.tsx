...
export default function AdminDashboard({
  ...
  const [activeStatus, setActiveStatus] =
    useState<JobApplication["status"]>("pending");
  ...
  const { data: employerInquiries = [], isLoading: isLoadingInquiries } =
    useQuery({
  ...
  const updateJobMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Job }) =>
      updateJob(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["jobs"] });
    },
  ...
  const filteredApplications = applications.filter((app) => {
    ...
    const matchesJob =
      !filters.selectedJob || app.job_id === filters.selectedJob;
    const matchesDateRange =
      (!filters.dateRange.start ||
        new Date(app.applied_date) >= new Date(filters.dateRange.start)) &&
      (!filters.dateRange.end ||
        new Date(app.applied_date) <= new Date(filters.dateRange.end));
    const matchesSkills =
      !filters.skills ||
      (app.cover_letter &&
    ...
    const matchesAiScore =
      aiScore >= filters.aiScoreRange.min &&
      aiScore <= filters.aiScoreRange.max;
  ...
  const isLoading =
    isLoadingJobs || isLoadingApplications || isLoadingInquiries;
                        ...
                        <div className="mt-2 space-y-1">
                          ...
                          {application.documents.length > 2 && (
                            <p className="text-xs text-gray-500">
                              +{application.documents.length - 2} more documents
                            </p>
                          )}
                        </div>
                      ) : (
